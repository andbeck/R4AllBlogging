...
Why change? It's consistent, fast, effective and makes working with data and graphs for data wrangling, data science and plain old science more easy and more fun like.  We think. But it is not required, and not the end of the world if someone doesn't know or use it.
...
But, getting to the point where you do  statistics - e.g. organising your data and making a good figure before you start analysing the data - well, this has changed for tidypeople.  But in reality, the reality is that there are just now more ways to do things than before. 

This blog post is designed to highlight some of what has changed. It is also designed to help you in 'working with dinosauRs'.  What follows is a set of examples of doing things in base R and dplyr or ggplot2.  It is not exhaustive.  Just a primer for liaising with dinosauRs.
...
Lets get setup with the packages and the data:
...
Oh my... we are already using base and dplyr!

#### SELECT a column with dplyr
...
# or
# select(compensation, 2) # gets the second column, which is Fruit!
```
...
#### Subset or Filter by a Variable using base R
...
#### Subset or Filter by a Variable using dplyr
...
Notice the similarities between `subset()` and `filter()`.  

#### Filter and select... the grand challenge
...
# Base Method 2: subset() + $
subset(compensation, subset = Fruit > 80)$Root
...
#### The dinosauR may do something like this.
